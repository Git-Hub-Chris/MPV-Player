...
among the presented items, and key bindings listed in `CONSOLE`_ are extended with
the following:
...
``history_date_format``
    ...
    The format of dates shown in ``select-watch-history``. This is passed to
    Lua's ``os.date``, which uses the same formats as ``strftime(3)``.
