...
mpv has a builtin conditional profile called ``builtin-image`` that is
automatically applied to images and restored when switching to a video or audio
file. It enables options optimal for image viewing, such as a smaller OSC layout
optimized for images. Its full contents can be listed with ``mpv
...
section in ``input.conf``, for example:
